...
for i in range(i_0,N,steps):
    ...
    #df = pd.read_csv(os.path.join(ROOT_DATA,"EVal_sig_1_i" + str(i)), header=None, names=columns)[0:N_evals].reset_index()
...
plt.savefig(os.path.join(ROOT_FIGURES, 'EVal_gs_varyN_10-800_on_-100_100'))
plt.show()